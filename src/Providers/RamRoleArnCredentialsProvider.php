...
class RamRoleArnCredentialsProvider extends SessionCredentialsProvider
{
    ...
    public function refreshCredentials()
    {
        ...
        $dateNew = substr($date, 0, 10);
        $dateNew = str_replace(
            "-",
            "",
            $dateNew
        );
        $region = Request::getRegion($this->stsEndpoint);
        $hashedRequestPayload = Request::hexEncode(Request::hashWithSha256(Helper::toBytes("")));
        $options['headers']["x-acs-content-sha256"] = $hashedRequestPayload;
        $options['headers']["Authorization"] = Request::getAuthorization("/", "GET", $options['query'], $options['headers'], $hashedRequestPayload, $sessionCredentials->getAccessKeyId(), $sessionCredentials->getAccessKeySecret(), $hashedRequestPayload, $region, $dateNew);
